...
export function createServer() {
  const server = new Server(
    { name: "mcp-server-redshift", version: "0.1.0" },
    { capabilities: { tools: {} } }
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req: CallToolRequest) => {
    ...
    switch (req.params.name) {
      case "list_tables":       result = await handleListTables(args.schemas as string[]); break;
      case "get_tables_schema": result = await handleGetTablesSchema(args.tables as any); break;
      case "query":             result = await handleQuery(args.sql as string); break;
